...
{
    CCLOG("SocketClient::~SocketClient");
	m_iState = SocketClient_DESTROY;
	if( m_hSocket!=-1)
		close(m_hSocket);
...
//            sendBuff.put(&msg, 0, msg->datalength());
			sendBuff->flip();
            CCLog("%d",socket);
            CCLog("%s   %d",(char *)sendBuff->getBuffer(),sendBuff->getLimit());
			int ret = send(socket,(char *)sendBuff->getBuffer(),sendBuff->getLimit(),0);
			if(ret == -1){
				This->m_iState = SocketClient_DESTROY;
				string err("发送数据，网络异常！");
...
//                            CCLOG("message->data == %s",message->data);
                            
//                            memcpy(message->data+0,&message->HEAD0,1);
//                            memcpy(message->data+1,&message->HEAD1,1);
//                            memcpy(message->data+2,&message->HEAD2,1);
//                            memvcpy(message->data+3,&message->HEAD3,1);
//                            memcpy(message->data+4,&message->ProtoVersion,1);
//                            memcpy(message->data+5,&message->serverVersion,4);
//                            memcpy(message->data+9,&message->length,4);
//                            memcpy(message->data+13,&message->commandId,4);
//                            memcpy(message->data+17,tmp,strlen(tmp));
                            
//							message->data = tmp;
//							message->datalength = length;
//							CCLog("receive message %0x \n",message->type);
							{
//								if (message->type == 0x8002A017|| message->type == 0x87777777 ||
//									message->type == 0x66666666 || message->type == 0x8002A016 ||
//									message->type == 0x8000E002 || message->type == 0x8002A015 || 
//									message->type == 0x0002A027 || message->type == 0x76666669 ||
...
//                                }else{
                               ...
                               // message->retain();
                                ...
                                if(bytesToInt(message->commandId)==218){
                                    ...
                                    int comid=bytesToInt(message->commandId);
                                    CCLog("%d",comid);
                                    char noticecomid[20]="";
                                    sprintf(noticecomid, "%d",comid);
                                    GameData::shareGameData()->dic->setObject(message, comid);
                                    CCNotificationCenter::sharedNotificationCenter()->postNotification(noticecomid,message);
                                }
